...
public class First {
    public static void main(String[] args) {
        // 1. DB 접속 정보
        // => JDBC 프로토콜
        //      - jdbc:사용하는DBMS://서버주소:포트번호(MysSQL기본3306)/DB명
...
//            pstmt.setString(2, "zxc123@example.com");
//
//            int result = pstmt.executeUpdate(); // INSERT 실행 - 반환은 영향받은 행의 수
//            System.out.println(result + "명 회원이 추가되었습니다 :)");
...
//            pstmt.setInt(1, 2); // parameter 동적 파라미터의 위치값 - 1부터 시작
//
...
//            System.out.println(updateResult + "명 수정되었습니다 :)");
            ...
            System.out.println(deletedResult + "명 삭제되었습니다 :)");

        } catch (Exception e) {
            System.out.println("DB 와의 커넥션 중 예외 발생");
            e.printStackTrace(); // - 예외 발생 시 예외 메세지 + 호출 스택 정보 출력 (예외 발생시 출력)
        } finally {
            // 자원 해제 (자바 7버젼 이상 - 자동 자원 해제) => 생략 가능
            try {
