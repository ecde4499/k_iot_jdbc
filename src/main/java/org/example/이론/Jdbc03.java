...
/*
    ...
    3. Statement, PreparedStatement 객체 생성 -  SQL 실핼 객체 생성
    ...
    1) SQL 작성 방법
     - Statement: SQL 직접 작성
     - PreparedStatement: ?를 사용하여 파라미터 처리

    2) 보안
    - Statement: SQL Injection 위험 있음
    - PreparedStatement: SQL Injection 방지

    cf) SQL Injection: 악의적인 SQL문 실행 (SQL 쿼리를 일반 입력 또는 양식 필드에 삽입하여 보안 취약성 공격)

    3) 성능
    - Statement: 매컨 컴파일
    - PreparedStatement: 사전 컴파일 => 속도가 더 빠름

    === executeQuery VS executeUpdate ===
    1) executeQuery
    : SELECT 문 실행 시 사용
    - 쿼리 실행 후 결과를 ResultSet 객체로 변환
    - ResultSet 내부에서 데이터 추출

    => ResultSet 결과 처리 방법
        - rs.next(): 다음 행 존재 여부 확인 및 이동
        - re.getInt("컬럼명"), rs.getString("컬럼명"), rs.getDate("컬럼명")
            : 정수값, 문자열 값, 날짜 값 추출
