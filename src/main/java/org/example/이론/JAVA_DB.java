...
/*
    ...
    : Java(SpringBoot)에서 DB와 연동하기 위한 방법

    1) JDBC (JDBC API 기반)
        : Java 에서 DB에 접근할 수 있도록 제공하는 표준 API
        - SQL 문을 문자열로 작성
    ...
    2) JPA (ORM, Object Relational Mapping 기반)
        ...
        : 자바 객체와 DB 테이블 간의 매핌을 지원하는 ORM 기술 표본
        - SQL을 직접 작성하지 않고, 자바 코드만으로 DB 연동 가능
        - SpringBoot 에서는 주로 Hibernate를 JPA 구현체로 사용
    ...
    3) MyBatis (SQL Mapper 기반)
        : SQL을 XML 또는 어노테이션으로 관리할 수 있는 SQLMapper 프레임 워크
